<template>
    <div class="taglist_box" v-loading.fullscreen.lock="fullscreenLoading">
        <div
            class="full_screen black"
            style="background:rgba(41, 216, 148, 1.0);"
            v-if="!is_mounted"
        >
            <img
        ...
        <!-- <div class="width_100 photntsback">
            <div class="phtonsbase overflow_scroll nscrol" id="sdcss">
                <div
                    v-if="all_type_arr.length"
                   :style="{width:all_type_arr.length * (pcOrPhone?200:100)+'px'}"
                    class="taglibox"
                >
                    <div
                        :class="{dsfx:index==INDEX}"
                        :style="{width:(pcOrPhone ? 200 : 100)+'px'}"
                        v-for="(item,index) in all_type_arr"
                        class="float_left text_center ittemstage"
                        :key="item.id"
                    >
                        {{item.name}}
                        <div
                            class="cate_border_bottom"
                            :class="{selected_cate :  select_index == index}"
                        ></div>
                    </div>
                </div>
            </div>
        ...
        <div>
            ...
            <div class="margin_top_10">
                ...
                >
                    <div
                        @click="toAll(cate_obj.original_name, cate_obj.name)"
                        class="display_flex flex_align_center phone_text_center flex_jusify_space margin_bottom_2"
                    >
                        <div class="font_size_21 cursor">{{ cate_obj.name }}</div>
                        ...
                        >
                            ...
                            <div class="hoverdiv hover_box transition_4s">
                                <div class="white width_100 text_top position_absolute">
                                    <div class="font_size_6 text_center">{{ cate_obj.arr[0].title }}</div>
                                    <div class="box_button margin_top_20">Watch It</div>
                                </div>
                            </div>
                        </div>
                        <div
                            class="display_flex wrop flex_1 flex_jusify_space right_div_box"
                            v-if="cate_obj.arr"
                        >
                            <video-div
...
export default {
    ...
    computed: {
        all_load() {
            debugger;
            return this.all_type_arr.every(v => {
                return v.arr;
            });
        }
    },
    methods: {
        ...
        moves(e, index, id) {
            ...
            var screleft =
                e.target.offsetLeft - this.$jquery("#sdcss")[0].scrollLeft;
        ...
        getList1() {
            this.fullscreenLoading = true;
            this.$http(
                "dcb/album/tag/" + this.arr[this.INDEX].id + "/",
                "get",
                {
...
.taglist_box {
    ...
    .photntsback {
        ...
        .dsfx {
            //color:@glore;
            font-weight: 600;
            background:rgba(234, 176, 176, 1.0);
        }
    }
